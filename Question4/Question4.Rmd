...
subset_summer <- data.frame(
  ...
  City = Summer$City,
  Sport = Summer$Sport,
  Discipline = Summer$Discipline,
  Country = Summer$Country,
  Gender = Summer$Gender,
  Event = Summer$Event,
  Medal = Summer$Medal
)
# Identify all duplicated rows to create an event type, true is an individual event and false is a team event
subset_summer <- subset_summer %>%
  mutate(Event_Type = duplicated(subset_summer) | duplicated(subset_summer, fromLast = TRUE))

# Convert logical values to binary (1 for TRUE, 0 for FALSE)
subset_summer <- subset_summer %>%
  mutate(Event_Type = as.integer(Event_Type))

# Identify all duplicated rows
subset_summer <- distinct(subset_summer)
...
# Identify all duplicated rows
subset_winter <- distinct(subset_winter)
head(subset_winter)
```

```{r}
# Adding summer and winter indicators
...
merged_olympics <- bind_rows(subset_winter,subset_summer) %>% arrange(Year)
```
```{r}
GDP <-  GDP %>% 
    drop_na() %>% 
...
TIP: the data shows medals per person,and some events (like hockey) show multiple winners (whole team) while the country receives only one medal. Be creative in your code and analysis and account for this as far as possible.
