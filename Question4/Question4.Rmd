...
```{r}
subset_summer <-  data.frame(Summer$Year,Summer$City,Summer$Sport, Summer$Discipline, Summer$Country, Summer$Gender, Summer$Event, Summer$Medal)
duplicated(subset_summer)
```


```{r}
#Labeling each entry whether it is individual or team event  
dataset_test <- Summer %>% 
    group_by(Year) %>% mutate(type = ifelse(str_detect(Event, "Team"),
                                                 "Team", "Individual"))
#Classifying medals based on team or individual events
dataset_test_medals <- dataset_test %>% filter(!is.na(Medal)) %>% group_by(type) %>%
  summarise(Medals = n())

print(dataset_test_medals)
...
classify_event_type <- function(df) {
  df <- df %>%
    ...
    mutate(
      type = ifelse(
        all(Medal == "Gold") | all(Medal == "Silver") | all(Medal == "Bronze"),
        "Team", "Individual"
      )
    ) %>%
    ungroup()
...
# Apply the function to classify events
dataset_test <- Summer %>%
  group_by(Year) %>%
  mutate(
    type = ifelse(str_detect(Event, "Team"), "Team", "Individual")
  ) %>%
  ungroup()

# Classifying medals based on team or individual events
dataset_test_medals <- dataset_test %>%
  filter(!is.na(Medal)) %>%
  group_by(type) %>%
  summarise(Medals = n())

# Print the summarized medal counts
print("Medal Counts Summary:")
print(dataset_test_medals)
```


TIP: the data shows medals per person,and some events (like hockey) show multiple winners (whole team) while the country receives only one medal. 
Be creative in your code and analysis and account for this as far as possible.

So what I could do is make a column that says its a team sport, then add the country and the medal? This could work I crack on it now.
