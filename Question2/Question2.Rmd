...
title: "Timeless Tunes: Tracing the Evolution and Endurance of Musical Legends"
...
```
```{r}
# The Metallica Dataset has duration in ms and I would like it in seconds to match Coldplay:
# Convert
metallica <- metallica %>% 
    rename(duration = duration_ms)  %>%  mutate(duration = duration/1000)
...
```{r}
#Let's have a look at the data shallwe: 

variable.names(billboard_100)
```
```{r}
variable.names(coldplay)
```
```{r}
variable.names(metallica)
```
```{r}
...
```{r}
# Checking if variable names match between coldplay and metallica: 

varlist <- list("name" , "duration", "release_date","album_name","explicit", "popularity","acousticness","danceability", "energy" ,"instrumentalness" ,"liveness", "loudness",  "speechiness" , "tempo" ,"valence" )

varlist %in% names(metallica) # names() function gets column names of dataframe
```
So it would appear that album_name is not in the metallica set the equivalent one is just album, and we do not have a variable for explicit in the metallica dataset which is not surprsing since all would be true anyway. 

I will rename the variable album_name to album in the coldplay dataset
```{r}
coldplay <- 
    coldplay %>% rename(album = album_name)
```
```{r}
head(coldplay,5)
head(billboard_100,5)
head(spotify,5) # year is double not date
head(metallica,5)
```
Billboard from 1958 and spotify from 1900
coldplay starts at 2000
metallica starts at 1983
I think there is something that I can do with the date columns here as well..

Maybe try add a column for each c and m to show whether live, demo, audition, remix, etc. would need to get a list of those..
